...
- 用户在当前对话里给出自然语言任务
- 任务写入线上 backend（`https://gtd.5666.net`）
- 新 Mac（bridge_id=`fh-macbook-air`）上的 `mac-sync-bridge` 成功拉到 remote push item
- bridge 成功生成 ack 并回传服务端
- 用户已在新 Mac 的 Apple Reminders 中肉眼确认看到了该提醒事项

本轮关键验证样例：

- 输入：`明晚8点提醒我给张三发合同`
- 线上任务 ID：`da48a082-207b-4103-a2d2-7cb5b656babe`

本轮 debug-sync 成功特征：

- `push_response_items_count=1`
- `ack_items_count=1`
- `report_acked=1`

这意味着：

**“当前对话 -> backend -> 新 Mac 本地提醒事项”的最小闭环已验证成功。**

### 本轮修复过的关键问题

1. assistant 入口创建的 task 需要正确进入 sync pending 轨道
2. bridge push cursor 不能用 `lastPushCursor` 提前跳过未 ack item，应改为基于 `lastAckedChangeID`
3. `mac-sync-bridge` 需要打印 push request/raw response，便于定位线上 contract 问题
4. remote push items 需要先落地本地 reminder upsert，再生成 ack
